     ...
     in perf "codeLens" $
            response $ do
                ...
                {- Normalise CPP/LHS files/custom preprocessed files.
                   Used to extract tests correctly from CPP and LHS (Bird-)
                   Literate Modules (.lhs) with begin{code}
                      Tests must be in
                  -}
                session :: HscEnvEq <-
                    perf "session" $
                        liftIO $
                            runAction "runEvalCmd.ghcSessionDeps" st $
                                use_ GhcSessionDeps $
                ...
                let text =
                        cleanSource (isLiterate fp) . decodeUtf8 $
                            stringBufferToByteString
                                ppContent
                -- dbg st "PREPROCESSED CONTENT" text
                        ...
                        (either Left id <$>)
                            ...
                            $ evalGhcEnv (hscEnvWithImportPaths session) $
                                do
                                    ...
                                    dbg st "importPaths0" impPaths0
                                    dbg st "importPaths1" impPaths1
                                    dbg st "importPaths2" impPaths2
                                    dbg st "importPaths" impPaths
                                    ...
                                    dbg st "QUICKCHECK NEEDS" $ needsQuickCheck tests
                                    dbg st "QUICKCHECK HAS" $ hasQuickCheck df
...
evals (st, fp) df stmts = do
  ...
  where
    -- dbg st = dbg st_ st
    -- eval :: TEnv -> DynFlags -> Statement -> Ghc (Maybe [Text])
    eval (Located l stmt)
...
-- dbg st_ :: (MonadIO m, Show a) => IdeState -> Text -> a -> m ()
-- dbg st_ st n v = debug "Eval" st n (T.pack . show $ v)
