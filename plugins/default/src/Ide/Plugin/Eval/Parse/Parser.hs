...
module Ide.Plugin.Eval.Parse.Parser(Parser,runParser,satisfy,alphaNumChar,letterChar,space,string,char,tillEnd) where
...
-- runParserRec m s = case parse m s of
--   Just (res, []) -> Right res
--   Just (res, ts) -> res <> runParserRec m
--   Nothing -> Left "No match"
